...
dataset_file_name = {
    ...
    "a3d":"dataset_A3D_contraxFormat.csv",
    "a3d_tst_neutral_rewrite":"dataset_A3Dtest_neutralized_rewrite_contraxFormat.csv",
    "a3d_tst_neutral_mt":"dataset_A3Dtest_neutralized_mt_contraxFormat.csv",
    "a3d_tst_gemini":"tst_gemini_contraxFormat.csv",
    "a3d_tst_gpt":"tst_gpt_contraxFormat.csv",
    "a3d_tst_deepseek":"tst_deepseek_contraxFormat.csv",
    "a3d_tst_llama":"tst_llama_contraxFormat.csv",
    "a3d_tst_fanar":"tst_fanar_contraxFormat.csv",
    "a3d_tst_jais":"tst_jais_contraxFormat.csv"
}
...
def build_train_test(df, source, limit, per_author=None, seed=None):
    ...
    if source.startswith('a3d'):
        ...
        full_test = sub_df[sub_df["train"] == 0]
        test_dict = full_test[['content', 'Target']] #in-document development set

        full_valid = sub_df[sub_df["train"] == 2]
        val_dict = full_valid[['content', 'Target']] #in-document tuning set

        full_testBlind = sub_df[sub_df["train"] == 3]
        testBlind_dict = full_testBlind[['content', 'Target']] #in-document test set

        full_test_out = sub_df[sub_df["train"] == 5]
        test_out_dict = full_test_out[['content', 'Target']] #cross-document development set

        full_valid_out = sub_df[sub_df["train"] == 4]
        val_out_dict = full_valid_out[['content', 'Target']] #cross-document tuning set

        full_testBlind_out = sub_df[sub_df["train"] == 6]
        testBlind_out_dict = full_testBlind_out[['content', 'Target']] #cross-document test set
        
        return full_train, val_dict, test_dict, testBlind_dict, val_out_dict, test_out_dict, testBlind_out_dict
