...
if __name__ == '__main__':
    datasets = ['imdb62', 'blog', 'turing','a3d']
    ...
    if 'a3d' in source:
        ...
        #train_bert(nlp_train, nlp_test, nlp_testBlind, nlp_valOut, nlp_testOut, nlp_testBlindOut, args.tqdm, args.model, 768, args.id, args.epochs, base_bs=7, base_lr=1e-5,mask_classes=mask_classes[args.dataset], coefficient=args.coe, num_authors=num_authors,val_dic=nlp_val)
        train_bert(nlp_train, nlp_test, args.tqdm, args.model, 768, args.id, args.epochs, base_bs=7, base_lr=1e-5,
                   mask_classes=mask_classes[args.dataset], coefficient=args.coe, num_authors=num_authors,val_dic=nlp_val, 
                   testBlind_dict=nlp_testBlind, valOut_dict=nlp_valOut, testOut_dict=nlp_testOut, testBlindOut_dict=nlp_testBlindOut)
    elif 'enron' in source or 'imdb62' in source or 'blog' in source:
